...
def check_for_retrieved_df():
    ...
    if len(retrieved_path) == 0:
        ...
        if len(glob.glob(f"./output/crossref/p/*.p", recursive=True)) > 0:
            print(
                "Converting the pickle files from the crossref directory to json files"
            )
            ...
            for i in range(len(all_ps)):
                ...
                with open(
                    f"./output/crossref/json/{all_ps[i].split('/')[-1].split('.')[0]}.json",
                    "w",
                ) as outfile:
        ...
        if len(glob.glob(f"./output/medline/p/*.p", recursive=True)) > 0:
            print(
                "Converting the pickle files from the medline directory to json files"
            )
            ...
            for i in range(len(all_ps)):
                ...
                json_object = json.dumps(temp_p, indent=4)
                with open(
                    f"./output/medline/json/{all_ps[i].split('/')[-1].split('.')[0]}.json",
                    "w",
                ) as outfile:
                    outfile.write(json_object)
                outfile.close()
        ...
        if len(glob.glob(f"./output/retrieved_df/*.p", recursive=True)) > 0:
            print(
                "Converting the pickle files from the retrieved_df directory to json files"
            )
            ...
            for i in range(len(all_ps)):
                ...
                with open(
                    f"./output/retrieved_df/{all_ps[i].split('/')[-1].split('.')[0]}.json",
                    "w",
                ) as outfile:
                    outfile.write(json_object)
                outfile.close()
        ...
        if len(glob.glob(f"./output/retrieved_df/*.json", recursive=True)) > 0:
            print(
                "Converting the json files from the retrieved_df directory to zip files"
            )
            all_ps = glob.glob(f"./output/retrieved_df/*.json", recursive=True)
            for i in range(len(all_ps)):
                zipfile.ZipFile(f"{all_ps[i]}.zip", mode="w").write(
                    f"{all_ps[i]}",
                    arcname=f'{all_ps[i].split("/")[-1].split(".")[0]}.json',
                )
                os.remove(f"{all_ps[i]}")

        if len(glob.glob(f"./output/retrieved_df/*.tsv", recursive=True)) > 0:
            print(
                "Converting the tsv files from the retrieved_df directory to zip files"
            )
            all_ps = glob.glob(f"./output/retrieved_df/*.tsv", recursive=True)
            for i in range(len(all_ps)):
                zipfile.ZipFile(f"{all_ps[i]}.zip", mode="w").write(
                    f"{all_ps[i]}",
                    arcname=f'{all_ps[i].split("/")[-1].split(".")[0]}.tsv',
                )
                os.remove(f"{all_ps[i]}")
        ...
        if len(glob.glob(f"./output/esearch_results/*.json", recursive=True)) > 0:
            print(
                "Converting the json files from the esearch_results directory to zip files"
            )
        ...
        if len(glob.glob(f"./output/medline/txts/*.txt", recursive=True)) > 0:
            ...
            for i in range(len(all_ps)):
                zipfile.ZipFile(f"{all_ps[i]}.zip", mode="w").write(
                    f"{all_ps[i]}",
                    arcname=f'{all_ps[i].split("/")[-1].split(".")[0]}.txt',
                )
                os.remove(f"{all_ps[i]}")
        ...
        if (
            len(
                glob.glob(
                    f"./output/retrieved_parsed_files/content_text/*.txt",
                    recursive=True,
                )
            )
            > 0
            ...
            print(
                "Converting the txt files from the retrieved_parsed_files/content_text directory to zip files"
            )
            all_ps = glob.glob(
                f"./output/retrieved_parsed_files/content_text/*.txt", recursive=True
            )
            for i in range(len(all_ps)):
                zipfile.ZipFile(f"{all_ps[i]}.zip", mode="w").write(
                    f"{all_ps[i]}",
        ...
        if (
            len(
                glob.glob(
                    f"./output/retrieved_parsed_files/htmls/*.txt", recursive=True
                )
            )
            > 0
        ):
            print(
                "Converting the txt files from the retrieved_parsed_files/htmls directory to zip files"
            )
            all_ps = glob.glob(
                f"./output/retrieved_parsed_files/htmls/*.txt", recursive=True
            )
        ...
        if (
            len(
                glob.glob(f"./output/retrieved_parsed_files/pdfs/*.txt", recursive=True)
            )
            > 0
        ):
            print(
                "Converting the txt files from the retrieved_parsed_files/pdfs directory to zip files"
            )
            all_ps = glob.glob(
                f"./output/retrieved_parsed_files/pdfs/*.txt", recursive=True
            )
            for i in range(len(all_ps)):
                zipfile.ZipFile(f"{all_ps[i]}.zip", mode="w").write(
                    f"{all_ps[i]}",
        ...
        if (
            len(
                glob.glob(f"./output/retrieved_parsed_files/txts/*.txt", recursive=True)
            )
            > 0
        ):
            print(
                "Converting the txt files from the retrieved_parsed_files/txts directory to zip files"
            )
            all_ps = glob.glob(
                f"./output/retrieved_parsed_files/txts/*.txt", recursive=True
            )
            for i in range(len(all_ps)):
                zipfile.ZipFile(f"{all_ps[i]}.zip", mode="w").write(
                    f"{all_ps[i]}",
        ...
        if (
            len(
                glob.glob(f"./output/retrieved_parsed_files/xmls/*.txt", recursive=True)
            )
            > 0
        ):
            print(
                "Converting the txt files from the retrieved_parsed_files/xmls directory to zip files"
            )
            all_ps = glob.glob(
                f"./output/retrieved_parsed_files/xmls/*.txt", recursive=True
            )
            for i in range(len(all_ps)):
                zipfile.ZipFile(f"{all_ps[i]}.zip", mode="w").write(
                    f"{all_ps[i]}",
        ...
        if len(glob.glob(f"./output/formats/htmls/*.html", recursive=True)) > 0:
            print(
                "Converting the html files from the formats/htmls directory to zip files"
            )
            all_ps = glob.glob(f"./output/formats/htmls/*.html", recursive=True)
            for i in range(len(all_ps)):
                zipfile.ZipFile(f"{all_ps[i]}.zip", mode="w").write(
                    f"{all_ps[i]}",
                    arcname=f'{all_ps[i].split("/")[-1].split(".")[0]}.html',
                )
                os.remove(f"{all_ps[i]}")

        if len(glob.glob(f"./output/formats/pdfs/*.pdf", recursive=True)) > 0:
            print(
                "Converting the pdf files from the formats/pdfs directory to zip files"
            )
            all_ps = glob.glob(f"./output/formats/pdfs/*.pdf", recursive=True)
            for i in range(len(all_ps)):
                zipfile.ZipFile(f"{all_ps[i]}.zip", mode="w").write(
                    f"{all_ps[i]}",
                    arcname=f'{all_ps[i].split("/")[-1].split(".")[0]}.pdf',
                )
                os.remove(f"{all_ps[i]}")

        if len(glob.glob(f"./output/formats/txts/*.txt", recursive=True)) > 0:
            print(
                "Converting the txt files from the formats/txts directory to zip files"
            )
        ...
        if len(glob.glob(f"./output/formats/xmls/*.xml", recursive=True)) > 0:
            print(
                "Converting the xml files from the formats/xmls directory to zip files"
            )
            all_ps = glob.glob(f"./output/formats/xmls/*.xml", recursive=True)
            for i in range(len(all_ps)):
                zipfile.ZipFile(f"{all_ps[i]}.zip", mode="w").write(
                    f"{all_ps[i]}",
                    arcname=f'{all_ps[i].split("/")[-1].split(".")[0]}.xml',
                )
                os.remove(f"{all_ps[i]}")
    ...
    if len(glob.glob("./output/medline/txts/temp_medline_output.txt.zip")) > 0:
        os.rename(
            "./output/medline/txts/temp_medline_output.txt.zip",
            "./output/medline/txts/medline_output.txt.zip",
        )
