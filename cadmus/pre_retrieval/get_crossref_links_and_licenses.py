...
def get_crossref_links_and_licenses(
    ...
    for index, row in cr_df.iterrows():
        ...
        response_d, response = get_request(
            row["doi"], http, base_url, headers, "base", elsevier_api_key
        )
        ...
        if response_d["status_code"] == 200:
            ...
            with zipfile.ZipFile(
                f"./output/crossref/json/{index}.json.zip",
                mode="w",
    ...
    for index, row in retrieved_df.iterrows():
        if row["full_text_links"].get("cr_tdm") != []:
            count += 1
    print(
        f"Out of the {len(cr_df)} articles with a doi, {sum(retrieved_df.crossref ==1)} were found in crossref"
    )
    print(
        f"We have found {count} crossref records with at least one TDM link available"
    )
    return retrieved_df
