...
def df_eval(df):
    ...
    for index, row in df.iterrows():
        if row["html"] == 1 or row["pdf"] == 1 or row["xml"] == 1 or row["plain"] == 1:
            # we retrieved at least one format
            retrieved += 1

        if (row["html"] == 1 or row["xml"] == 1) and (row["pdf"] == 1):
    ...
    # printing the result
    print(
        f"Out of {len(df)} articles\nwe got at least one version for {retrieved} = {pc(retrieved, len(df))}\n"
    )
    print(
        f"We got a pdf and tagged version for {criteria} artilces = {pc(criteria,len(df))}\n"
    )
