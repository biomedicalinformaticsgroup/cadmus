...
def retrieval(
    ...
    if stage == "crossref":
        ...
        exit()
                    ...
                    # trying to indentifying the format from the link in order to not request a document if it is already retreive for that format
                        ...
                        if response_d["status_code"] == 429:
                            ...
                            if limit_reset:
                                ...
                                limit_remaining = headers.get(
                                    "CR-TDM-Rate-Limit-Remaining"
                                )
                                if limit_remaining:
                                    if int(limit_remaining) == 0:
                                        print(
                                            "download limit reached, need to back off"
                                        )
                                    ...
                                    if (
                                        ...
                                        try:
                                            ...
                                            if (
                                                ...
                                                and pdf_d["wc"] != 0
                                                and (
                                                    pdf_d["wc_abs"] < pdf_d["wc"]
                                                    or pdf_d["wc_abs"] > 1000
                                                    if pdf_d["wc_abs"] != 0
                                                    else True
                                                )
                                                and 100 < pdf_d["wc"]
                            ...
                            # trying to indentify if the link will provide the algorithm with a xml format
                                ...
                                if retrieval_df.at[index, "xml"] != 1:
                                    ...
                                    if (
                                        xml_d["evaluation"] == "TP"
                                        and (
                                            xml_d["wc_abs"] < xml_d["wc"]
                                            or xml_d["wc_abs"] > 1000
                                            if xml_d["wc_abs"] != 0
                                            else True
                                        )
                                        and 100 < xml_d["wc"]
                                        ...
                                        with zipfile.ZipFile(
                                            f"./output/formats/xmls/{index}.xml.zip",
                                            mode="w",
                                ...
                                if retrieval_df.at[index, "html"] != 1:
                                    ...
                                    if (
                                        html_d["evaluation"] == "TP"
                                        and (
                                            html_d["wc_abs"] < html_d["wc"]
                                            or html_d["wc_abs"] > 1000
                                            if html_d["wc_abs"] != 0
                                            else True
                                        )
                                        and 100 < html_d["wc"]
                                        ...
                                        with zipfile.ZipFile(
                                            f"./output/formats/htmls/{index}.html.zip",
                                            mode="w",
                                            compression=zipfile.ZIP_DEFLATED,
                                            compresslevel=9,
                                        ) as zip_file:
                            ...
                            # doing the same as before for .txt file format
                                ...
                                if retrieval_df.at[index, "plain"] != 1:
                                    ...
                                    if (
                                        plain_d["wc"] != 0
                                        and (
                                            plain_d["wc_abs"] < plain_d["wc"]
                                            or plain_d["wc_abs"] > 1000
                                            if plain_d["wc_abs"] != 0
                                            else True
                                        )
                                        and 100 < plain_d["wc"]
            ...
            if (
                retrieval_df.xml.loc[index] != 1
                and retrieval_df.pmcid.loc[index] != None
            ):
                try:
                    # creating the header and the protocol to retreive the file from epmc API
                    response_d, response = get_request(
                        pmcid, http, base_url, headers, "epmcxml", elsevier_api_key
                    )
                except:
                    pass
                ...
                # if status code is 200, it means everything works correctly
                    ...
                    if (
                        xml_d["evaluation"] == "TP"
                        and (
                            xml_d["wc_abs"] < xml_d["wc"] or xml_d["wc_abs"] > 1000
                            if xml_d["wc_abs"] != 0
                            else True
                        )
                        and 100 < xml_d["wc"]
            ...
            if (
                ...
                try:
                    ...
                    response_d, response = get_request(
                        pmcid, http, base_url, headers, "pmcxmls", elsevier_api_key
                    )
                except:
                    pass
            ...
            if (
                retrieval_df.pdf.loc[index] != 1
                and retrieval_df.pmcid.loc[index] != None
            ):
                try:
                    # creating the header and the protocol to request the docuemnt from PMC API
                    response_d, response = get_request(
                        pmcid, http, base_url, headers, "pmcpdfs", elsevier_api_key
                    )
                except:
                    pass
            ...
            if (
                retrieval_df.pmc_tgz.loc[index] != 1
                and retrieval_df.pmcid.loc[index] != None
            ):
                try:
                    # creating the header and protocol to request the tgz from PMC
                    response_d, response = get_request(
                        pmcid, http, base_url, headers, "pmctgz", elsevier_api_key
                    )
                except:
                    pass
                        ...
                        if link.attrs["format"] == "tgz":
                            ...
                            if worked == True:
                                try:
                                    ...
                                    retrieval_df = timeout(300)(tgz_unpacking)(
                                        index,
                                        retrieval_df,
            ...
            if retrieval_df.doi.loc[index] != None:
                try:
                    ...
                    response_d, response = get_request(
                        doi, http, base_url, headers, "doiorg", elsevier_api_key
                    )
                except:
                    pass
                ...
                if response_d["status_code"] == 429:
                    ...
                    if len(response_d["content"]) > 100:
                        ...
                        if format_type == "":
                            print(
                                "No format Type in the headers, trying to extract from soup object"
                            )
                        ...
                        if format_type != None:
                            ...
                            if (
                                "pdf" in format_type.lower()
                                and retrieval_df.pdf.loc[index] != 1
                            ):
                                if response_d["headers"].get("Content-Type") != None:
                                ...
                                "xml" in format_type.lower()
                                and retrieval_df.xml.loc[index] != 1
                            ):
                                # perform xml parsing and FP detection
                                xml_d, p_text = xml_response_to_parse_d(
                                    retrieval_df, index, response, keep_abstract
                                ...
                                and retrieval_df.html.loc[index] != 1
                            ):
                                ...
                                and retrieval_df.plain.loc[index] != 1
                            ):
                                with zipfile.ZipFile(
                                    f"./output/formats/txts/{index}.txt.zip",
                                    mode="w",
        ...
        # no html page are saved at this stage only candidate links
            ...
            if retrieval_df.pmid.loc[index] == retrieval_df.pmid.loc[index]:
                ...
                if (
                    (retrieval_df.html.loc[index] == 0)
                    and (retrieval_df.xml.loc[index] == 0)
                ) or (retrieval_df.pdf.loc[index]) == 0:
                    # send the request to pubmed using the base url and pmid
                    try:
                        response_d, response = get_request(
                            pmid, http, base_url, headers, "pubmed", elsevier_api_key
                        )
                    except:
                        pass
        ...
        if counter % 10 == 0 and not counter % 100 == 0 and not counter == 0:
            ...
            clear()
            print(
                f'In case of faillure please put the parameters start="{saved_stage}" (or "{saved_stage}_only" if in only mode) and idx="{saved_index}"'
            )
            print("\n")
