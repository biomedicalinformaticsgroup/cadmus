...
def bioscraping(
    ...
    if update:
        ...
        if "mesh" not in original_df.columns:
            print(
                "Implementing changes to your previous result due to change in the library."
            )
        ...
        if (
            original_df.iloc[0].content_text == 0
            or original_df.iloc[0].content_text == 1
        ):
            pass
        else:
            print(
        ...
        if full_search == "heavy":
            ...
            for index, row in original_df.iterrows():
                ...
                if (row["pdf"] == 1 and row["html"] == 1) or (
                    row["pdf"] == 1 and row["xml"] == 1
                ):
                    # removing these pmids from the search
                    original_pmids.append(row["pmid"])
                else:
    ...
    if type(input_function) == str or input_function[0].isdigit() == True:
        print(
            "This look like a search string or list of pmids. \nIf this is not correct Please stop now"
        )
        ...
        if start != None:
            try:
                ...
                with zipfile.ZipFile(
                    "./output/retrieved_df/retrieved_df.json.zip", "r"
                ) as z:
                    for filename in z.namelist():
                        with z.open(filename) as f:
                            d = f.read()
                ...
                print(
                    f"You don't have any previous retrieved_df we changed your parameters start and idx to None"
                )
                start = None
                idx = None
        ...
        if start == None:
            if input_function == "":
                ...
                if type(input_function) == str:
                    ...
                    if type(input_function) == list:
                        if 9000 < len(input_function):
                            print(
                                "Your list of PMIDs is greater than 9000, creating bins of 9000."
                            )
                        chunks = [
                            (",").join(input_function[x : x + 9000])
                            for x in range(0, len(input_function), 9000)
                        ]
                ...
                retrieved_df = retrieved_df.drop_duplicates(
                    keep="last", ignore_index=False, subset=["doi", "pmid"]
                )
        ...
        if update:
            ...
            if len(new_pmids) == 0:
                ...
                if start == None:
                    print(
                        "There are no new lines since your previous search - stop the function."
                    )
                    exit()
                else:
                    pass
            ...
            if idx != None and start == None:
                print(
                    f"You can't have your parameter idx not equal to None when start = None, changing your idx to None"
                )
                idx = None
        ...
        if start == None:
            ...
            http, base_url, headers = HTTP_setup(
                email, click_through_api_key, wiley_api_key, "base"
            )
            ...
            retrieved_df["full_text_links"] = [
                {"cr_tdm": [], "html_parse": [], "pubmed_links": []}
                for value in retrieved_df.index
            ]
            ...
            retrieved_df = get_crossref_links_and_licenses(
                retrieved_df, http, base_url, headers, elsevier_api_key
            )
            ...
            results_d = {
                ...
                "search_term": input_function,
                "total_count": len(list(np.unique(retrieved_df.pmid))),
                "pmids": list(np.unique(retrieved_df.pmid)),
            }
            with zipfile.ZipFile(
                f"./output/esearch_results/{datetime.datetime.now().strftime('%Y_%m_%d_%H_%M_%S')}.json.zip",
                mode="w",
                compression=zipfile.ZIP_DEFLATED,
                compresslevel=9,
            ) as zip_file:
                ...
                zip_file.writestr(
                    f"{datetime.datetime.now().strftime('%Y_%m_%d_%H_%M_%S')}.json",
                    data=dumped_JSON,
                )
                zip_file.testzip()
            zip_file.close()
        ...
        if start == None and idx == None:
            http, base_url, headers = HTTP_setup(
                email, click_through_api_key, wiley_api_key, "crossref"
            )
            # now use the http request set up to request for each of the retrieved_df
            retrieved_df = retrieval(
                retrieved_df,
                http,
                base_url,
                headers,
        ...
        # we run the code only on crossref
            ...
            try:
                ...
                with zipfile.ZipFile(
                    "./output/retrieved_df/retrieved_df2.json.zip", "r"
                ) as z:
                    for filename in z.namelist():
                        with z.open(filename) as f:
                            d = f.read()
            ...
            if idx != None:
                try:
                    ...
                    print(
                        f"The idx you enter was not found in the retrieved_df, please enter a correct index"
                    )
                    exit()
                if divide_at != 0:
        ...
        # we start at the crossref step and at a specific index, could be related to a previous failled attempt
            ...
            if divide_at != 0:
                ...
                retrieved_df = pd.concat(
                    [done, finish], axis=0, join="outer", ignore_index=False, copy=True
                )
        ...
        if start == None and idx == None:
            http, base_url, headers = HTTP_setup(
                email, click_through_api_key, wiley_api_key, "doiorg"
            )
            # now use the http request set up to request for each of the retrieved_df
            retrieved_df = retrieval(
                retrieved_df,
        ...
        if start == None and idx == None:
            http, base_url, headers = HTTP_setup(
                email, click_through_api_key, wiley_api_key, "epmcxml"
            )
            # now use the http request set up to request for each of the retrieved_df
            retrieved_df = retrieval(
                retrieved_df,
        ...
        if start == None and idx == None:
            http, base_url, headers = HTTP_setup(
                email, click_through_api_key, wiley_api_key, "pmcxmls"
            )
            # now use the http request set up to request for each of the retrieved_df
            retrieved_df = retrieval(
                retrieved_df,
        ...
        if start == None and idx == None:
            http, base_url, headers = HTTP_setup(
                email, click_through_api_key, wiley_api_key, "pmctgz"
            )
            # now use the http request set up to request for each of the retrieved_df
            retrieved_df = retrieval(
                retrieved_df,
        ...
        if start == None and idx == None:
            http, base_url, headers = HTTP_setup(
                email, click_through_api_key, wiley_api_key, "pmcpdfs"
            )
            # now use the http request set up to request for each of the retrieved_df
            retrieved_df = retrieval(
                retrieved_df,
        ...
        if start == None and idx == None:
            http, base_url, headers = HTTP_setup(
                email, click_through_api_key, wiley_api_key, "pubmed"
            )
            # now use the http request set up to request for each of the retrieved_df
            retrieved_df = retrieval(
                retrieved_df,
        ...
        if start == None or start == "mid_retrieval":
            clear()
            print(
                f'In case of faillure please put the parameters start="mid_retrieval", it might take a while depending on the size of your search'
            )
            ...
            eval_retrieved_df = retrieved_df[
                ["pdf", "html", "plain", "xml", "content_text", "abstract"]
            ]
        ...
        if start == None and idx == None:
            ...
            retrieved_df2 = parse_link_retrieval(
                retrieved_df,
                email,
                click_through_api_key,
                keep_abstract,
                wiley_api_key,
                elsevier_api_key,
            )
        ...
        if start == None or start == "post_retrieval":
            print(
                f'In case of faillure please put the parameters start="post_retrieval", it might take a while depending on the size of your search'
            )
            ...
            if update:
                ...
                retrieved_df2 = pd.concat(
                    [original_df, retrieved_df2],
                    axis=0,
                    join="outer",
                    ignore_index=False,
                    copy=True,
                )
                retrieved_df2 = retrieved_df2.drop_duplicates(
            ...
            if len(glob.glob("./output/retrieved_df/retrieved_df2.json.zip")) == 0:
                with zipfile.ZipFile(
                    "./output/retrieved_df/retrieved_df2.json.zip",
                    mode="w",
                    compression=zipfile.ZIP_DEFLATED,
                    compresslevel=9,
                ) as zip_file:
                ...
                os.rename(
                    "./output/retrieved_df/retrieved_df2.json.zip",
                    "./output/retrieved_df/temp_retrieved_df2.json.zip",
                )
                with zipfile.ZipFile(
                    "./output/retrieved_df/retrieved_df2.json.zip",
                    mode="w",
            ...
            if len(glob.glob("./output/retrieved_df/retrieved_df2.tsv.zip")) == 0:
                retrieved_df2.to_csv(
                    f"./output/retrieved_df/retrieved_df2.tsv.zip",
                    sep="\t",
                    compression="zip",
                )
            else:
                os.rename(
                    "./output/retrieved_df/retrieved_df2.tsv.zip",
                    "./output/retrieved_df/temp_retrieved_df2.tsv.zip",
                )
                retrieved_df2.to_csv(
                    f"./output/retrieved_df/retrieved_df2.tsv.zip",
                    sep="\t",
        ...
        print(
            "Your input is not handle by the function please enter Pubmed search terms or a list of single type(dois, pmids, pmcids), without header"
        )
