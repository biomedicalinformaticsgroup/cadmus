        ...
        try:
            ...
            if retrieval_df.at[index, "pdf"] != 1:
                ...
                pdfs = [
                    path
                    for path in Path(
                        f"./output/formats/tgzs/temp_dir/{index}/{pmc}"
                    ).iterdir()
                    if path.suffix == ".pdf"
                ]
                ...
                if pdf_path:
                    ...
                    try:
                        ...
                        if (
                            ...
                            and p_text != ""
                            and (
                                pdf_d["wc_abs"] < pdf_d["wc"] or pdf_d["wc_abs"] > 1000
                                if pdf_d["wc_abs"] != 0
                                else True
                            )
                            and 100 < pdf_d["wc"]
            ...
            if retrieval_df.at[index, "xml"] != 1:
                ...
                xmls = [
                    ...
                    if (path.suffix == ".nxml") or (path.suffix == ".xml")
                ]
                ...
                for xml in xmls:
                    ...
                    if retrieval_df.at[index, "xml"] != 1:
                        ...
                        with open(str(xml), "r") as xml_file:
                            ...
                            parse_d.update(
                                {
                                    "file_path": f"./output/formats/xmls/{index}.xml.zip",
                                    "size": size,
                                    "wc": wc,
                                    "wc_abs": wc_abs,
                            ...
                            if (
                                parse_d["evaluation"] == "TP"
                                and (
                                    parse_d["wc_abs"] < parse_d["wc"]
                                    or parse_d["wc_abs"] > 1000
                                    if parse_d["wc_abs"] != 0
                                    else True
                                )
                                and 100 < parse_d["wc"]
                                ...
                                xml.rename(to_file)
                                zipfile.ZipFile(
                                    f"./output/formats/xmls/{index}.xml.zip", mode="w"
                                ).write(
                                    f"./output/formats/xmls/{index}.xml",
                                    arcname=f"{index}.xml",
                                )
