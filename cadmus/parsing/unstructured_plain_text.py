...
def unstructured_plain_text(p_text, keep_abstract):

    if keep_abstract == True:
        # as we retrieved txt files from our previous usage, we identified two types of structured when getting .txt
            ...
            if p_text.lower().count("## references") == 2:
                ...
                for i in range(len(p_text.split())):
                    if p_text.split()[i] == "##" and count < 2:
                        lower = i + 1
                        count += 1
            # looking for the reference only in th second half of the text
            while upper > int(len(p_text.split()) / 2):
                if (
                    p_text.split()[upper].lower() == "references"
                    or p_text.split()[upper].lower() == "funding"
                    or p_text.split()[upper].lower() == "acknowledgement"
                    or p_text.split()[upper].lower() == "acknowledgements"
                ):
                    break
                upper -= 1
        ...
        # letter to the editor, often a reply to an initial publication
            ...
            upper = len(p_text.split()) - 1
            while upper > 0:
                # the content is from to the editor to the references
                if (
                    p_text.split()[upper].lower() == "references"
                    or p_text.split()[upper].lower() == "funding"
                    or p_text.split()[upper].lower() == "acknowledgement"
    ...
    if keep_abstract == False:
        # as we retrieved txt files from our run, we identified two type of strutured when getting .txt
        if p_text.lower().count("##") > 2:
            lower = 0
            upper = len(p_text.split()) - 1
            # if the tag ## references is present two times, the content is the text between the two tags
        ...
        # letter to the editor, often a reply to an initial publication
            ...
            upper = len(p_text.split()) - 1
            while upper > int(len(p_text.split()) / 2):
                # in case we don't find any sturture, we want to find teh upper limit to be the reference
                if p_text.split()[upper].lower() == "references":
                    break
                upper -= 1
            if upper == int(len(p_text.split()) / 2):
            ...
            for i in range(len(p_text.split())):
                ...
                if (
                    "introduction" in p_text.lower().split()[i]
                    or "abstract" in p_text.lower().split()[i]
                    or "letters" in p_text.lower().split()[i]
                    or "correspondence" in p_text.lower().split()[i]
                    or "editorial" in p_text.lower().split()[i]
                ):
                    lower = i
                    key_present = True
                    break
