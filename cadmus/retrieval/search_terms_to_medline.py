    ...
    if colab1 == True or colab2 == True:
        ...
        if os.path.isfile("./.bash_profile") == False:
            if os.path.isdir("./output/medline/edirect") == False:
                ...
                bash_profile_file = [path_to_edirect, f"export NCBI_API_KEY={api_key}"]
        ...
        if type(query_string) == str:
            search_results = pipeline(
                f'esearch -db pubmed -query "{query_string}" | efetch -format medline'
            )
            if len(glob.glob(f"./output/medline/txts/*.zip")) == 0:
                ...
                with zipfile.ZipFile(
                    "./output/medline/txts/medline_output.txt.zip", "r"
                ) as z:
            ...
            for i in range(len(query_string)):
                search_results = pipeline(
                    f'esearch -db pubmed -query "{query_string[i]}" | efetch -format medline'
                )
                if len(glob.glob(f"./output/medline/txts/*.zip")) == 0:
                    with zipfile.ZipFile(
                        "./output/medline/txts/medline_output.txt.zip",
                    ...
                    os.remove("./output/medline/txts/temp_medline_output.txt.zip")
                print(
                    f"Medline Records retrieved and saved {i+1} out of {len(query_string)}"
                )
